...
def student_list(request):
    ...
    #Aggregate function demo here

    # average = qs.aggregate(Avg('marks'))
    # sum = qs.aggregate(mysum=Sum('marks'))#for cutomized  name for aggregated value
    # min = qs.aggregate(Min('marks'))
    # max = qs.aggregate(Max('marks'))
    # count = qs.aggregate(Count('marks'))
    # print(average,sum,min,max,count)
    ...
    qs=Student.objects.filter(Q(name='chiku') & Q(roll=2))  
    qs=Student.objects.filter(Q(name='chiku') | Q(roll=1))  
    qs=Student.objects.filter(~Q(name='chiku'))  




    print(qs.query,qs)
    # return render(request,'school/studentall.html',{'students':qs,'average':average,'sum':sum,'min':min,'max':max,'count':count})
